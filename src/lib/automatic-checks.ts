...
export function gydFloridaSignals(prizes: [string, string, string], session: "midday" | "evening") {
...
export type SourceDraw = { date: string; session: "midday" | "evening"; pick3: string; pick4: string };
...
export function sourcePairs(draw: SourceDraw): string[] {
  if (!isValidDate(draw.date) || !/^\d{3}$/.test(draw.pick3) || !/^\d{4}$/.test(draw.pick4)
    || !["midday", "evening"].includes(draw.session)) throw new Error("Rezilta sous la pa konplè oswa li pa valab.");
  return [...new Set([
...
export function scanOldWorkout(sources: [SourceDraw, SourceDraw], target: Pick<SourceDraw, "date" | "session">, db: Database): AutoComparison[] {
  if (!isValidDate(target.date) || !["midday", "evening"].includes(target.session)) throw new Error("Tiraj sib la pa valab.");
  const key = (d: Pick<SourceDraw, "date" | "session">) => `${d.date}-${d.session === "midday" ? 0 : 1}`;
